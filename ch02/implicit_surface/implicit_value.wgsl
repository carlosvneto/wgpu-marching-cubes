@group(0) @binding(0) var<storage, read_write> valueBuffer : array<f32>;
...
fn cs_main(@builtin(global_invocation_id) id : vec3u) {
    ...
    let idx = getIdx(id);
    valueBuffer[idx] = implicitFunc(position.x, position.y, position.z, fps.animateTime, ips.funcSelection);
}
