...
use wgpu_marching_cubes::wgpu_simplified as ws;
...
impl State {
    pub async fn new(
        ...
        let mut rng_sample_range = || rng.generate::<f32>();

        let mut metaball_positions = vec![];

        for _ in 0..metaballs_count {
            metaball_positions.push(MetaballPosition {
                x: -4.0 * (2.0 * rng_sample_range() -1.0),
                y: -4.0 * (2.0 * rng_sample_range() -1.0),
                z: -4.0 * (2.0 * rng_sample_range() -1.0),
                vx: 1000.0 * rng_sample_range(),
                vy: 10.0 * (2.0 * rng_sample_range() -1.0),
                vz: 1000.0 * rng_sample_range(),
                speed: 2.0 * rng_sample_range() + 0.3,
            });
        }
    ...
    pub fn update(&mut self) {
        ...
        let mut rng = WyRand::new_seed(elapsed.as_secs());
        let mut rng_sample_range = || rng.generate::<f32>();

        if elapsed >= std::time::Duration::from_secs(5) {
            self.subtract_target = 3.0 * rng_sample_range() + 3.0;
            self.strength_target = 3.0 * rng_sample_range() + 3.0;
        }    
    }
